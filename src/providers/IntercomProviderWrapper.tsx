...
export const IntercomProviderWrapper = ({children}: IntercomProviderWrapperProps) => {
    ...
    const representativeName = contactRepresentatives && contactRepresentatives[0]?.name
    ...
    const { data } = useQuery<DocumentsByNameQuery>(
        DOCUMENTS_BY_NAME_QUERY,
        {
            variables: {
    ...
    const intercomSettingsFile = data?.documents?.find(document => document.fileName === INTERCOM_SETTINGS_FILE)
    const intercomSettingsFileContent = intercomSettingsFile && JSON.parse(window.atob(intercomSettingsFile.data))
    INTERCOM_APP_ID = intercomSettingsFileContent?.appId ? intercomSettingsFileContent?.appId : ""
